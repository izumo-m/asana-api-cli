...
def _header(equivalent_args: list[str]) -> list[str]:
    """The two-line provenance / traceability header (C-5).
    ...
    (read from ``sys.argv``, minus ``--generate-python``). No sanitizing: any
    secret the user put on the command line already appears in the transcribed
    body, so the comment adds no new exposure.
    """
    return [
        f"# Generated by asana-api {version.version_string()}",
        f"# Equivalent to: {shlex.join(['asana-api', *equivalent_args])}",
    ]
