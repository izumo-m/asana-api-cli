...
@click.group("users", cls=GroupWithGlobalOptions)
...
@formatted
def get_favorites_for_user(user: str, resource_type: str, workspace: str | None, offset: str | None, opt_fields: str | None, all_items: bool, paginate: bool, page_size: int | None, max_items: int | None) -> Any:
    ...
    if max_items is not None:
        return session.fetch_capped(api.get_favorites_for_user, user, resource_type, resolved_workspace, opts=opts, max_items=max_items)
...
@click.option("--max-items", "max_items", type=int, default=None, help="Stop after fetching this many items in total")
@formatted
def get_users(workspace: str | None, offset: str | None, opt_fields: str | None, team: str | None, all_items: bool, paginate: bool, page_size: int | None, max_items: int | None) -> Any:
    """Get multiple users"""
    resolved_workspace = resolve_workspace(workspace, required=False)
    if paginate:
        click.echo("Warning: --paginate is deprecated; use --all-items instead.", err=True)
    ...
    if resolved_workspace is not None:
        opts["workspace"] = resolved_workspace
    if max_items is not None:
        return session.fetch_capped(api.get_users, opts=opts, max_items=max_items)
