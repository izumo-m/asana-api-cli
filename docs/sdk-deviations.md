...
| `--output-errors` / `--query-errors` (v3.1+) | (Not in SDK — Python raises an exception per HTTP error) | `--output-errors {raw\|json\|text\|csv\|table}` (default `raw`). `raw` lets the exception propagate uncaught — Python's default handler prints the traceback on stderr and the process exits `1` (the SDK-parity baseline). `json` / `text` / `csv` / `table` catch the exception, render an envelope on **stdout**, and exit `3`. `ApiException` produces a 5-field envelope `{exception, status, reason, body, headers}` where `exception` is the qualified `module.qualname` (e.g. `"asana.rest.ApiException"`), `body` is the UTF-8 decoded response *string* (or `null`), and `headers` is the response header dict. Other exceptions raised from the SDK call path (e.g. `urllib3.exceptions.MaxRetryError`) collapse to a 2-field `{exception, reason}` — no HTTP context. The `exception` field always uses the qualified import path so SDK users can `from <module> import <class>` to handle the same error in their own code. `--query-errors EXPR` applies a `jq` filter to the envelope and renders each yield per `--output-errors`; passing it with the default `raw` emits a stderr warning ("the filter is being ignored") but does not block the call — preserving the underlying error rather than masking it with a usage error. Click's own errors (`ClickException`, `Abort`, `Exit`) are not wrapped — they keep Click's standard handling. Same renderer as `--output`; nested dict / list values in text/csv/table render as JSON (`{"a":"b"}`), not Python repr. Envelope on stdout (rather than stderr) means scripts can rely on `case $? in 0) ... ;; 3) ... ;; esac` and consume stdout uniformly, independent of whatever noise urllib3 or other libraries put on stderr | `raw` default keeps SDK parity for casual one-off use; opting into an envelope is an opt-in scripts make explicitly. Stdout output + the `exit_code == 3` discriminator gives a clean machine-readable error channel | `_handle_exception` + `_format_output` in `formatter.py` |
| Exit code on failure | Python defaults | `0` success, `1` uncaught exception from the SDK call (default `--output-errors=raw`), `2` user-input invalid, `3` envelope-rendered API / connection error. See [`exit-codes.md`](exit-codes.md) | Lets scripts distinguish kinds of failures | `_handle_exception` in `formatter.py`; `sys.exit(2)` sites in `session.py` |
