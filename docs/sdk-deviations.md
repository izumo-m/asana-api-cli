...
| Default pagination return shape | `return_page_iterator=True` yields a lazy iterator (`PageIterator` / `EventIterator`) | The iterator is walked to completion (`list(result)`) inside the session context and printed as a flat list ([`usage.md`](usage.md#pagination)) | (1) An unwalked iterator cannot be serialized to stdout. (2) Materializing inside the session keeps `HttpClientAuthRedactor` active for every page request; lazy iteration after the session exits would leak `Authorization` headers on pages 2..N (constitution #2) | post-judge `isinstance` check in `execute_call_plan` in `cli.py` |
...
| `--generate-python` (v3.2+) | (Not in SDK — it runs the call) | Print a standalone python-asana script equivalent to the command instead of executing it; no token, no network ([`usage.md`](usage.md#generating-python-code)) | Shell ergonomics / learning: turns a working invocation into copy-pasteable SDK code. Reproduces the configuration, the call, output / error formatting, `--debug` (with the `Authorization` redactor, so the generated trace stays masked — constitution #2), and uploads. Credentials (`--access-token`, `Authorization` headers, the `--proxy` password) are masked in the emitted text — constitution #2; see [SECURITY.md](../SECURITY.md) | `render_python` in `codegen.py`; `runtime.generate_python` consumed in `formatter.py:formatted` |
...
| `--all-items` / `--page-size` / `--max-items` | (Not in SDK) | v2-era deprecation aliases of the v3 flags ([`usage.md`](usage.md#deprecated-aliases)) | v2 → v3 migration path; scheduled for removal in a future release | `_apply_deprecated_aliases` in `cli.py` (warning + forwarding), called from `build_call_plan`; option injection in the `paginatable` block of `_make_command` |
| SDK arg/opt name colliding with a built-in CLI flag | The SDK has the param; flags are a CLI-only namespace | The colliding SDK arg/opt is exposed as `--sdk-<name>`; the built-in keeps its bare name and the SDK param keeps its real name in dest / call / label | The CLI adds extension flags that share the `--` namespace; on collision the SDK param must stay reachable (constitution #1 (b)), and prefixing the SDK side keeps every built-in flag stable across commands (no per-command polysemy) | `_static_reserved_flags` / `_decls` in `cli.py`; `tests/test_cli.py::TestFlagCollisions` |
