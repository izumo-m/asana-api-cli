...
class TestGeneratedCli:
    def test_header(self, tasks_code: str) -> None:
        ...
        assert "from asana_api_cli.click_ext import GroupWithGlobalOptions" in tasks_code
    ...
    def test_group_decorator(self, tasks_code: str) -> None:
        assert '@click.group("tasks", cls=GroupWithGlobalOptions)' in tasks_code
...
class TestCliInit:
    def test_imports(self, groups: list[ApiGroup]) -> None:
        ...
        assert "from asana_api_cli.click_ext import LazyGroup" in code

    def test_lazy_subcommands_map(self, groups: list[ApiGroup]) -> None:
        code = generate_cli_init(groups)
        # Each group is registered via the lazy map, not via eager import.
        assert '"tasks": ("asana_api_cli.cli.tasks:tasks_group", "Tasks commands."),' in code
        assert (
            '"workspaces": ("asana_api_cli.cli.workspaces:workspaces_group", '
            '"Workspaces commands."),'
        ) in code
        # No eager imports of subcommand modules.
        assert "from asana_api_cli.cli.tasks import" not in code

    def test_main_group(self, groups: list[ApiGroup]) -> None:
        code = generate_cli_init(groups)
        assert "@click.group(cls=LazyGroup, lazy_subcommands=LAZY_SUBCOMMANDS)" in code
    ...
    def test_no_add_command_calls(self, groups: list[ApiGroup]) -> None:
        # Subcommands are wired via LazyGroup.lazy_subcommands; add_command
        # would defeat lazy loading by triggering eager imports.
        code = generate_cli_init(groups)
        assert "main.add_command(" not in code
